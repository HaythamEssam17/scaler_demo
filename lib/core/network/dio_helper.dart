...
// Instantiate the class if it hasn't been created yet.
  ...
  Future<Response> getDate({required String url}) async {
    try {
      ...
      /// Get custom massage for the exception
      devLog("here is the error from dio get data ${exception.response?.data["message"]} ");
      devLog("here is the error from dio get data ${exception.message ?? ''} ");
      final CustomStatusCodeErrorType errorType = DioExceptions.fromDioError(exception).errorType;
      String errorMessage = '';
  ...
  Future<Response> postData({required String url, required FormData data}) async {
    try {
      ...
      devLog("here is the error from dio post data ${exception.response?.data["message"]} ");
      ...
      devLog("this is error from CustomException exception${error.runtimeType}");
  ...
  Future<Response> putData({required String url, required FormData data}) async {
    try {
      ...
      devLog("here is the error from dio put data ${exception.response?.data["message"]} ");
  ...
  Future<Response> deleteData({required String url}) async {
    try {
      ...
      final errorMessage = DioExceptions.fromDioError(exception).errorMassage;
      if (DioExceptions.fromDioError(exception).errorType != CustomStatusCodeErrorType.unVerified) {}
  ...
  Future sendMessageToSlack(String messageText) async {
    try {
      ...
      dio.post(dotenv.env['slack_webHock_url'] ?? '', data: json.encode(request)).then(
        (response) {
